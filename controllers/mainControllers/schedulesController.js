...
export async function createSchedule(req, res) {
  try {
    const classroom = req.roles.includes("role.superAdmin") && req.body.classroom ? req.body.classroom : user.classroom._id;
...
export async function deleteSchedule(req, res) {
  ...
  try {
    ...
    if (!(req.user.roles.includes("role.superAdmin") || scheduleToBeDeleted.classroom._id.toString() == req.user.classroom._id.toString())) {
...
export async function updateSchedule(req, res) {
  try {
    const classroom = req.roles.includes("role.superAdmin") && req.body.classroom ? req.body.classroom : req.user.classroom._id;
    const schedule = {
      ...req.body, creator: req.user._id, classroom
    }
    ...
    if (!(req.user.roles.includes("role.superAdmin") || scheduleToBeUpdated.college._id.toString() == populatedUser.classroom.college._id.toString())) {
      return res.status(401).send({
