...
export async function sendMessage(req, res, next) {
  ...
  io.once("connection", (socket) => {
    socket.on("room", async roomid => {
      ...
      socket.broadcast.to(users.user.room).emit('message', `${req.user.Name} has joined the group`)
    ...
    socket.on('disconnect', () => {
      ...
      if (removedUser) {
        ...
        socket.broadcast.to(removedUser.room).emit('message', `${removedUser.username} has left the group`)
      }
    })
  });
